...
* **Correção de Engasgos/Estalos no Áudio (Buffer Underrun):**
  - O backend de áudio SDL3 no Android solicita por padrão modo de baixa latência (`LOW_LATENCY`), alocando buffers mínimos (~4 ms). Durante a decodificação pesada de vídeo VP6 e uploads de texturas, pequenas flutuações de tempo causavam *buffer underruns* imediatos. Corrigido desativando o modo de ultra-baixa latência (`SDL_ANDROID_LOW_LATENCY_AUDIO = "0"`), aumentando o buffer do dispositivo para 2048 quadros (`SDL_AUDIO_DEVICE_SAMPLE_FRAMES = "2048"`, ~42 ms de margem) e elevando `audio_maxqframes = 128`, eliminando cortes e estalos.
* **Correção de Crash no Startup / Troca de Activity (NullPointerException em DecorView.getWindowInsetsController):**
  - Em dispositivos com Android 11+ (API 30+) como Motorola Moto G100, ao transitar da `TitleActivity` para a `MainActivity`, o método `applyImmersiveStickyMode()` chamava `getWindow().getInsetsController()` antes que o `DecorView` interno da janela estivesse completamente montado pelo framework, disparando `NullPointerException` interno no `PhoneWindow.java:3979`. Corrigido obtendo o controlador via `window.getDecorView().getWindowInsetsController()`, protegendo chamadas assíncronas do Turnip com checagem nula de decor e envolvendo toda a configuração imersiva em bloco `try/catch` com fallback defensivo.
